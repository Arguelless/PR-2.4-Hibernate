...
public class Manager {
    ...
    public static Biblioteca addBiblioteca(String name, String city){
        Session session = factory.openSession();
        Transaction tx = null;
        Biblioteca result = null;
        try {
            tx = session.beginTransaction();
            result = new Biblioteca(name, city);
            session.save(result); 
            tx.commit();
        } catch (HibernateException e) {
            if (tx!=null) tx.rollback();
    ...
    public static void updateBiblioteca(long bibliotecaId, String name, String city, Set<Llibre> llibres){
        Session session = factory.openSession();
        Transaction tx = null;
        try {
            tx = session.beginTransaction();
